...
DecisionTree.prototype.initializeOps = function() {
  ...
  this.op(':=', this._collections.location_class_count, this._collections.training_data_loc.groupBy(
    ...
    function(data_loc) {
      return [data_loc[numFeatures+2], data_loc[numFeatures+1]];
    },
    function(k, xs) {
      return xs.first().concat(xs.count());
    }
  ), {
    target: 'location_class_count',
    nonMonotonicDeps: ['training_data_loc']
  });

  this.op(':=', this._collections.location_count, this._collections.location_class_count.groupBy(
    function(lcc) { return lcc[0]; },
    function(lcc) { return lcc[2]; },
    function(node_id, counts) {
      var sum = 0;
